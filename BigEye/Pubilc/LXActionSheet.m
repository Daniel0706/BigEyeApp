...
#define ACTIONSHEET_BACKGROUNDCOLOR             [UIColor colorWithWhite:1 alpha:0.8];
...
//    [self.backGroundView addGestureRecognizer:tapGesture];
    ...
    if (otherButtonTitlesArray) {
        if (otherButtonTitlesArray.count > 0) {
            ...
            if (self.isHadTitle == NO && self.isHadDestructionButton == NO) {
                ...
                UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, 30 , ScreenWidth-2*BUTTON_INTERVAL_WIDTH, 60 * otherButtonTitlesArray.count)];
                bgView.backgroundColor = OTHER_BUTTON_COLOR;
                ...
                for (int i = 0; i<otherButtonTitlesArray.count; i++) {
                    ...
                    UILabel *lineLabel = [[UILabel alloc] initWithFrame:CGRectMake(BUTTON_INTERVAL_WIDTH + 2, 29.5 + ((i+1)*(BUTTON_HEIGHT+(BUTTON_INTERVAL_HEIGHT/2))), ScreenWidth-2*BUTTON_INTERVAL_WIDTH - 4, 0.5)];
                    lineLabel.backgroundColor = SeparatorLineRGB;
    ...
    [UIView animateWithDuration:ANIMATE_DURATION animations:^{
        [self.backGroundView setFrame:CGRectMake(0, ([UIScreen mainScreen].bounds.size.height-self.LXActionSheetHeight)/2, [UIScreen mainScreen].bounds.size.width, self.LXActionSheetHeight)];
...
{
    UIButton *destructiveButton = [[UIButton alloc] initWithFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, BUTTON_INTERVAL_HEIGHT, ScreenWidth-2*BUTTON_INTERVAL_WIDTH, BUTTON_HEIGHT)];
...
{
    NSArray *imageArray = @[@"拍照icon",@"图片icon"];
    AppendixCustomButton *otherButton = [[AppendixCustomButton alloc] initWithFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, 30 + (postionIndex*(BUTTON_HEIGHT+(BUTTON_INTERVAL_HEIGHT/2))), ScreenWidth-2*BUTTON_INTERVAL_WIDTH, 60)];
...
//    otherButton.layer.borderColor = BUTTON_BORDER_COLOR;
    ...
    [otherButton setImage:[UIImage imageNamed:imageArray[postionIndex]] forState:UIControlStateNormal];
...
{
    UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, BUTTON_INTERVAL_HEIGHT, ScreenWidth-2*BUTTON_INTERVAL_WIDTH, BUTTON_HEIGHT)];
...
{
    NSLog(@"tappedBackGroundView");
    
    if (self.delegate) {
        if ([self.delegate respondsToSelector:@selector(tappedBackGroundViewDelegate)] == YES) {
            [self.delegate tappedBackGroundViewDelegate];
        }
    }
